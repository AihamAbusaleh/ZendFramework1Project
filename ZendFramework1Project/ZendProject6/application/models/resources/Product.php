...
class Application_Resource_Product extends Zend_Db_Table_Abstract {
    ...
    public function delProduct($id) {
        //  'prodId' è necessaria, in caso contrario, la colonna dell`intero prodotto verrà eliminata
        $this->delete('prodId =' .(int) $id);
    }
    
     public function updateProduct($id, $modyfingProduct ) {
          
            
            $this->update($modyfingProduct, 'prodId = '. (int)$id);                                                                                                                                  //Line 6
    }
