...
\item $A_2$ ist DEA
\item $A_3$ ist DEA
\item $A_4$ ist NEA, von S0 gehen zwei Übergänge mit einer Beschriftung nämlich a
...
\textbf{  Lösung
 ...
 \path[->] (A) edge [bend right] node[anchor=below,below] {1} (B)
  		   	   edge [loop above] node {0} (A) 
  		   (B) edge [bend right] node[anchor=above,above] {1} (A)
